...
serve(async (req) => {
  ...
  try {
    ...
    if (totalAmount <= 0) throw new Error("Invalid booking amount. Please ensure dates and pricing are configured.");
    ...
    console.log(`Creating checkout: ${totalAmount} ${currency} (${amountInCentavos} centavos), method: ${paymentMethod}`);
    ...
    // Update activation with payment session and billing info
    await supabase
      .from("activations")
      .update({
