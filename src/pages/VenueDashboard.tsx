...
const VenueDashboard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (!profileData) {
        toast({ title: "Profile Required", description: "Please complete your profile setup." });
                  ...
                  <div className="space-y-4">
                    {totalSlides > 1 && (
                      <div className="flex items-center justify-center gap-4 mb-4">
                        ...
                        <div className="flex gap-2">{Array.from({ length: totalSlides }).map((_, idx) => (<button key={idx} onClick={() => setCurrentSlide(idx)} className={`w-2 h-2 rounded-full ${idx === currentSlide ? "bg-primary" : "bg-muted-foreground/30"}`} />))}</div>
