...
export const CTABanner = () => {
  ...
  return (
    <section
      ref={sectionRef}
      className="py-12 md:py-24 bg-card border-y border-border relative overflow-hidden"
    >
      ...
      <div className="container mx-auto px-4 md:px-6 text-center space-y-4 md:space-y-8 relative z-10">
        <h2
          className={`text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold max-w-3xl mx-auto text-foreground transition-all duration-700 ${
        ...
        <div
          className={`flex flex-col sm:flex-row items-center justify-center gap-4 md:gap-8 transition-all duration-700 delay-200 ${
            isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
          }`}
        >
          <div className="text-center space-y-2 md:space-y-3 w-full sm:w-auto">
            ...
            <Link to="/publishers" className="block">
              <Button
                size="lg"
                className="w-full sm:w-auto text-base md:text-lg px-6 md:px-8 py-5 md:py-6 neon-glow hover:neon-glow-strong"
              >
                Buy Ad Space
              </Button>
            </Link>
