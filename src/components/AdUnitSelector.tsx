...
const AD_UNIT_TYPES = [
  { id: "sticker_spot", label: "Sticker Spot (2x2, 3x3, 4x4)" },
  { id: "door_poster", label: "Door Poster (A4 / A3)" },
  { id: "countertop_display", label: "Countertop Display" },
...
export const AdUnitSelector = ({ selectedUnits, onUnitsChange, publisherId }: AdUnitSelectorProps) => {
  ...
  const updateUnit = (typeId: string, updates: Partial<AdUnitConfig>) => {
    onUnitsChange(
      selectedUnits.map(u => (u.type === typeId ? { ...u, ...updates } : u))
    );
  };
  ...
  const generateThumbnail = async (typeId: string) => {
    ...
    setGeneratingThumbnails(prev => ({ ...prev, [typeId]: true }));

    try {
      const { data, error } = await supabase.functions.invoke('generate-ad-thumbnail', {
        body: { prompt, adUnitType: typeId },
      });

      if (error) throw error;

      if (data?.imageUrl) {
        updateUnit(typeId, { thumbnailUrl: data.imageUrl });
        toast({
          title: "Success",
      ...
      setGeneratingThumbnails(prev => ({ ...prev, [typeId]: false }));
    }
  };
            ...
            >
              <CardContent className="p-4">
                <div className="flex items-start gap-3">
                  <Checkbox
                    checked={selected}
                    onCheckedChange={() => toggleUnit(unitType.id)}
                    className="mt-1"
                  />
                  <div className="flex-1 space-y-3">
                    <label
                      className="font-medium cursor-pointer"
                      onClick={() => toggleUnit(unitType.id)}
                    >
                      {unitType.label}
                    </label>
                    ...
                    {selected && (
                      <div className="space-y-3 mt-3">
                        {unitType.id === "custom_format" && (
                          <div>
                            <Label className="text-xs">Custom Format Description</Label>
                            <Input
                              placeholder="Describe your custom ad format..."
                              value={config?.customFormat || ""}
                              onChange={(e) =>
                                updateUnit(unitType.id, { customFormat: e.target.value })
                              }
                              className="mt-1"
                            />
                          </div>
                        ...
                        <div className="grid grid-cols-3 gap-2">
                          <div>
                            ...
                            <Input
                              ...
                              onChange={(e) =>
                                updateUnit(unitType.id, { quantity: parseInt(e.target.value) || 1 })
                              }
                              className="mt-1"
                            />
                          </div>
                          <div>
                            ...
                            <Input
                              ...
                              min="0"
                              step="0.01"
                              value={config?.pricePerWeek || ""}
                              onChange={(e) =>
                                updateUnit(unitType.id, { pricePerWeek: parseFloat(e.target.value) || 0 })
                              }
                              className="mt-1"
                              placeholder="0"
                            />
                          ...
                          <div>
                            ...
                            <Input
                              ...
                              min="0"
                              step="0.01"
                              value={config?.pricePerMonth || ""}
                              onChange={(e) =>
                                updateUnit(unitType.id, { pricePerMonth: parseFloat(e.target.value) || 0 })
                              }
                              className="mt-1"
                              placeholder="0"
                            />
                        ...
                        <div>
                          ...
                          <Textarea
                            placeholder="Any special placement rules or restrictions..."
                            value={config?.specialRules || ""}
                            onChange={(e) =>
                              updateUnit(unitType.id, { specialRules: e.target.value })
                            }
                            className="mt-1"
                            rows={2}
                          />
                        ...
                        <div className="border-t pt-3 mt-3">
                          <div className="flex items-center justify-between mb-2">
                            ...
                            <Button
                              ...
                              size="sm"
                              variant="outline"
                              onClick={() => generateThumbnail(unitType.id)}
                              disabled={isGenerating || (unitType.id === "custom_format" && !config?.customFormat)}
                            >
                              {isGenerating ? (
                                <>
